...
def about():
    ...
    def __template__():
        ...
        yield '', join_('                  * <a href="javascript:(function(){var re = new RegExp(\'^http://(.*?)\\.com/questions/([0-9]+)/\');var group = re.exec(window.location.href);if (group!=null){var service = group[1];var questionid=group[2];window.open(\'http://stackprinter.appspot.com/export?format=HTML&service=\'+service+\'&question=\'+questionid)}else{alert(\'Attention: question id not found!\')}})()">Bookmarklet</a><br>\n')
        yield '', join_('                  <br>\n')
...
def export():
    ...
    def __template__ (service, question , answers, pretty_links, printer):
        ...
        yield '', join_('                <a href="/"><img title="Back to home" width="20px" heigth="20px" border="0" src="/images/icon_home.png"/></a>\n')
        yield '', join_('                <a href="http://stackprinter.appspot.com/export?format=HTML&service=', escape_(service, True), '&printer=false&question=', escape_((question['question_id']), True), '"><img title="Link to this printed question" width="30px" heigth="17px" border="0" src="/images/Share.gif"/></a>\n')
        yield '', join_('            </div>         \n')
...
def favorites_delicious():
    ...
    def __template__ (username, result):
        ...
        yield '', join_("                                title: '[Q]uicklook',\n")
        yield '', join_("                                width: '650px',\n")
        yield '', join_('                                modal: true,\n')
        yield '', join_("                                position: ['center', 95]});\n")
        yield '', join_("                        jQuerydialog.load('/quicklook?question='+ question_id +'&service='+service,'', function (responseText, textStatus, XMLHttpRequest) {\n")
        yield '', join_('                        }\n')
        yield '', join_('                );\n')
        yield '', join_('                return false;\n')
        yield '', join_('            }\n')
